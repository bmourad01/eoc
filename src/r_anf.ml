...
let rec rco = function
  | R.Program (info, e) ->
      let l, a = rco_exp String.Map.empty (ref 1) e in
      let e =
...
and rco_exp m n = function
  | R.Int i -> ([], Int i)
  | R.(Prim Read) ->
      let x = fresh_var n in
      ([(x, Prim Read)], Var x)
  | R.(Prim (Minus e)) ->
      let nv, a = rco_exp m n e in
      let x = fresh_var n in
      (nv @ [(x, Prim (Minus a))], Var x)
  | R.(Prim (Plus (e1, e2))) ->
      let nv1, a1 = rco_exp m n e1 in
      let nv2, a2 = rco_exp m n e2 in
      let x = fresh_var n in
      (nv1 @ nv2 @ [(x, Prim (Plus (a1, a2)))], Var x)
  | R.Var v -> (
    match Map.find m v with
    | None -> failwith ("R_anf.rco_exp: var " ^ v ^ " is unbound")
    | Some e -> ([], e) )
  | R.Let (v, e1, e2) ->
      let nv1, a1 = rco_exp m n e1 in
